...
assert.match(html, /js\/field-recovery-ui\.js/, 'módulo de recuperação deve carregar na aplicação');
...
assert.match(recovery, /Nunca sobrescreve uma edição já iniciada/, 'restauração não deve substituir entrada atual');
...
assert.match(sync, /if \(!navigator\.onLine\)[\s\S]*Offline · salvo localmente/, 'sync deve declarar salvamento local enquanto offline');
assert.match(sync, /window\.addEventListener\('online',[\s\S]*syncNow/, 'sync deve retomar automaticamente ao recuperar conexão');
assert.match(sync, /visibilitychange[\s\S]*syncNow/, 'sync deve retomar quando o app voltar ao primeiro plano');
assert.match(sync, /if \(activeSyncPromise\) return activeSyncPromise/, 'sync deve continuar single-flight após retomada');
